...
if docker compose exec ldap ldapsearch -x -H ldaps://localhost:636 -D "cn=admin,dc=passbolt,dc=local" -w P4ssb0lt -b "dc=passbolt,dc=local" "(cn=$GROUPNAME)" | grep -q "uniqueMember: cn=$USERNAME,ou=users,dc=passbolt,dc=local"; then
    echo "User '$EMAIL' is already a member of group '$GROUPNAME'"
    exit 0
fi

# Create LDIF for adding user to group
LDIF_FILE="/tmp/add_${USERNAME}_to_${GROUPNAME}.ldif"
cat > "$LDIF_FILE" << EOF
...
docker compose cp "$LDIF_FILE" ldap:/tmp/add_${USERNAME}_to_${GROUPNAME}.ldif
docker compose exec ldap ldapmodify -x -H ldaps://localhost:636 -D "cn=admin,dc=passbolt,dc=local" -w P4ssb0lt -f /tmp/add_${USERNAME}_to_${GROUPNAME}.ldif

# Clean up
docker compose exec ldap rm /tmp/add_${USERNAME}_to_${GROUPNAME}.ldif
rm "$LDIF_FILE"

echo "User '$EMAIL' added to group '$GROUPNAME'" 