#!/bin/bash
# Setup Passbolt with Traefik (full stack including LDAP, Keycloak, etc)
set -e

echo "Setting up Passbolt with Traefik"
echo "================================="
echo ""
...
wait_for_ldap() {
    ...
    while [ $counter -lt $timeout ]; do
        if docker compose -f docker-compose.traefik.yaml exec $container ldapsearch -x -H ldap://localhost:389 \
...
docker compose -f docker-compose.traefik.yaml down 2>/dev/null || true
echo "   Starting database, cache, and SMTP services..."
docker compose -f docker-compose.traefik.yaml up -d db valkey smtp4dev
...
docker compose -f docker-compose.traefik.yaml up -d ldap1 ldap2
...
export COMPOSE_FILE=docker-compose.traefik.yaml
...
docker compose -f docker-compose.traefik.yaml up -d ldap-meta
...
docker compose -f docker-compose.traefik.yaml up -d traefik
...
docker compose -f docker-compose.traefik.yaml up -d keycloak passbolt
...
docker compose -f docker-compose.traefik.yaml build passbolt
docker compose -f docker-compose.traefik.yaml up -d passbolt
sleep 10
...
if docker compose -f docker-compose.traefik.yaml exec passbolt su -m -c '/usr/share/php/passbolt/bin/cake passbolt register_user -u ada@passbolt.com -f "Ada" -l "Lovelace" -r admin' -s /bin/bash www-data 2>&1 | grep -q "already exists\|already registered"; then
...
echo "   docker compose -f docker-compose.traefik.yaml exec passbolt su -s /bin/bash -c \"/usr/share/php/passbolt/bin/cake directory_sync all --persist --quiet\" www-data"
...
echo ""
echo "Service Status:"
if docker compose -f docker-compose.traefik.yaml ps | grep -q "Up"; then
    docker compose -f docker-compose.traefik.yaml ps --format "table {{.Name}}\t{{.Status}}\t{{.Ports}}"
    echo ""
    echo "All services are running successfully."
else
    echo "Some services failed to start. Check logs with: docker compose -f docker-compose.traefik.yaml logs"
    exit 1
fi
